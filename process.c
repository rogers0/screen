...
# endif
      ...
      if (n == 0 || n > 7)
	{
      ...
      if (n > 5 && (
        ((args[0][5] < '0' || args[0][5] > '4') && args[0][5] != '.') ||
        ((args[0][6] < '0' || args[0][6] > '4') && args[0][6] &&
	  args[0][6] != '.')))
	{
...
InputAKA()
{
  char *s, *ss;
  int n;
  Input("Set window's title to: ", MAXAKA, INP_COOKED, AKAfin, NULL);
  s = fore->w_title;
  if (!s)
    return;
  for (; *s; s++)
